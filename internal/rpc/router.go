...
import (
	...
	procedure "github.com/threefoldtech/tf-grid-cli/internal/rpc/procedures"
)

type Router struct {
	routes map[string]func(ctx context.Context, data string) (response Response)
}

func (r *Router) Register(route string, fn func(context.Context, string) Response) error {
	if _, ok := r.routes[route]; ok {
		return fmt.Errorf("route %s is already mapped", route)
	}
...
func MachinesDeploy(ctx context.Context, data string) Response {
	...
	if err != nil {
		return Response{
			Err: errors.Wrap(err, "failed to deploy"),
		}
	}
